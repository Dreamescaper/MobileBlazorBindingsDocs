---
title: 'Walkthrough: Hello World - Maui Blazor Bindings'
ms.topic: tutorial
ms.prod: aspnet-core
---
...
Let's take a look at the initial project that was created in the previous walkthrough to understand more about how to use Experimental Maui Blazor Bindings.

The main project to look at is the shared project that contains the `.razor` files. The Android and iOS projects don't contain any code specific to Experimental Maui Blazor Bindings.
...
1. The markup that defines the UI elements and their associated properties and event handlers:
    ...
    The HTML-like tags represent UI components that match the MAUI components and their properties and events. Some properties have computed values, such as the `Label` component's `Text` property, which has its value set to a value computed by C# code, which is denoted by the `@( ... )` expression block.
